...
const Sphere = {
  ...
  create(box, scene) {
    ...
    const shape1 = new THREE.CircleGeometry(1, 10);
    const shape2 = new THREE.CircleGeometry(2, 20);
    const points = new THREE.SphereGeometry(100, 30, 14).vertices;
    ...
    for (let i = 0; i < points.length; i++) {
      ...
      const cycle = THREE.Math.randInt(0, 100);
      const pace = THREE.Math.randInt(10, 30);
      const shape = new THREE.Mesh((i % 2) ? shape1 : shape2, material);
  ...
  update(box, mouse, freq) {
    const bass = (Math.floor(freq[1] | 0) / 255);
    ...
    for (let i = 0; i < this.group.children.length; i++) {
      const shape = this.group.children[i];
      const {
        radius,
        cycle,
        pace,
...
Vue.filter('toSpaces', (str) => String(str || '').trim().replace(/[^\w\`\'\-]+/g, ' ').trim());
...
new Vue({
  ...
  computed: {
    ...
    channelsList() {
      ...
      const search = this.searchText.replace(/[^\w\s\-]+/g, '').replace(/[\r\s\t\n]+/g, ' ').trim();
    ...
    canPlay() {
      return !!((this.channel.id && !this.loading));
    },
    ...
    hasErrors() {
      return !!((this.checkError('init') || this.checkError('stream')));
    },
  },
  ...
  methods: {
    ...
    getChannels(sidebar) {
      const endpoint = 'https://somafm.com/channels.json';
      const emsg = ['There was a problem trying to load the list of available channels from SomaFM.'];
      ...
      axios.get(endpoint).then((res) => {
        ...
        for (const c of res.data.channels) {
          ...
          c.listeners |= 0; // force numeric
          c.updated |= 0; // force numeric
    ...
    fetchSongs(channel, cb) {
      ...
      const emsg = [`There was a problem trying to load the list of songs for channel ${  channel.title  } from SomaFM.`];
      ...
      axios.get(channel.songsurl).then((res) => {
        ...
        const songs = res.data.songs.slice();
    ...
    setupCanvas() {
      ...
      for (const o of this.fxObjects) o.create(this.fxBox, this.fxScene);
    ...
    updateStageLight() {
      const dist = Math.floor(this.freqData[1] | 0) / 255;
      const color = Math.floor(this.freqData[16] | 0) / 255;
